...
void patient::saveMap()
{
    ...
    f << "patientId,firstName,lastName,gender,age,mobNumber,address,height,weight,wasHospitalized?,stillAlive(ifHospitalized)?,room,illness\n";
    for (auto i : hospital::patientsList)
        f << i.second.id << "," << i.second.firstName << "," << i.second.lastName << "," << i.second.gender
          ...
          << (i.second.hospitalized ? 'Y' : 'N') << "," << (i.second.alive ? 'Y' : 'N') << "," << i.second.room << "," << i.second.illness << endl;
    f.close();
...
void patient::addPerson()
{
    ...
    cin >> weight;
    cout << "\nEnter the illness of the patient:\n";
    cin >> illness;
...
void patient::getDetailsFromHistory()
{
    ...
    if (opt == 1)
    {
        ...
        while (getline(f >> ws, temp))
        {
            ...
            //creating a string stream object to read from string 'temp';
            stringstream s(temp);
            string s3, s4, s6, s7, s8, s9, s10, s12, s13, s14;
            //reading from the string stream object 's';
            getline(s, p.firstName, ',');
            getline(s, p.lastName, ',');
...
void patient::hospitalize()
{
    ...
    {
        if (i.second.patient1Id == 0 || i.second.patient2Id == 0 || i.second.patient3Id == 0)
        {
            room = i.second.roomId;
            gotOne = 1;
            if(i.second.patient1Id == 0)
            {
                hospital::wardsList[room].patient1Id = id;
            }
            else if(i.second.patient2Id == 0)
            {
                hospital::wardsList[room].patient2Id = id;
            }
            else
            {
                hospital::wardsList[room].patient3Id = id;
            }
            break;
        }
    }
    if (!gotOne)
    {
        cout << "No idle ward found!"
             << "\n";
        return;
    }    
    hospital::patientsList[id].hospitalized = 1;
    hospital::patientsList[id].room = room;
    ...
    cout << firstName << " " << lastName << " hospitalized successfully!The ward is " << this->room << "\n";
    return;
}
void patient::reportADeath()
{
    ...
    if(hospital::patientsList[id].room != 0)
    {
        for (auto i : hospital::wardsList)
        {
            if (i.second.patient1Id == id)
            {
                i.second.patient1Id = 0;
                hospital::wardsList[i.second.roomId].patient1Id = 0;
                break;
            }
            else if (i.second.patient2Id == id)
            {
                i.second.patient2Id = 0;
                hospital::wardsList[i.second.roomId].patient2Id = 0;
                break;
            }
            else if (i.second.patient3Id == id)
            {
                i.second.patient3Id = 0;
                hospital::wardsList[i.second.roomId].patient3Id = 0;
                break;
            }
        }
    }
    hospital::patientsList[id].room = 0;
    string s, temp, corrected;
    stringstream str;
    fstream f, fout;
    str << firstName << "," << lastName
