...
void driver::reportLeave()
{
    cout << "\nSearch for the driver.\n";
    getDetails();
    if (id == -1)
        return;
    if(hospital::driversList[id].idle == 0)//正在执行任务，无法请假
    {
        cout<<"\nUnable to take leave. Please try again later.\n";
        return;
    }
    hospital::driversList[id].idle = 0;
    cout<<"\nHope you have a good holiday.\n";
    return;
}

void driver::endHoliday()
{
    cout << "\nSearch for the driver.\n";
    getDetails();
    if (id == -1)
        return;
    if(hospital::driversList[id].idle == 0)
    {
        for (auto i : hospital::ambulancesList)
        {
            if (i.second.idle == 0 && i.second.D.id == id)
            {
                cout<<"\nNot in holiday. Unsuccessfully!\n";
                return;
            }
        }
    }
    hospital::driversList[id].idle = 1;
